import math
from heuristic1 import evaluate_move, is_immediate_win
...
def minimax(board, depth, maximizing_player, captures, turn, last_move:tuple[int,int], before_last_move:tuple[int,int], num_moves:int, alpha=-math.inf, beta=math.inf):
    count=1
    ...
    inv_color=3-turn
    color=turn
    possible_moves = [pmove_s[0] for pmove_s in sorted([(pmove,evaluate_move(pmove[0], pmove[1], board, color if maximizing_player else inv_color, inv_color if maximizing_player else color, captures, num_moves)+(100 if is_immediate_win(pmove[0], pmove[1], board, color) else 0)+(100 if is_immediate_win(pmove[0], pmove[1], board, inv_color) else 0)) for pmove in possible_moves],key=lambda x:x[1])]
    if maximizing_player:
        ...
        for move in possible_moves:
            ...
            eval_score, _, new_count = minimax(board, depth - 1, False, new_captures, 3 - turn, move, last_move, num_moves, alpha, beta)
            count+=new_count
            board[row][col] = 0
        ...
        for move in possible_moves:
            ...
            positions_captured,new_captures = get_new_captures(board,color,captures,row,col)
            eval_score, _, new_count = minimax(board, depth - 1, True, new_captures, turn, move, last_move, num_moves, alpha, beta)
            count+=new_count
            board[row][col] = 0
            for pos in positions_captured:
                board[pos[0]][pos[1]]=inv_color
...
def do_ai(last_move:tuple[int,int],before_last_move:tuple[int,int],board:list[list[int]],captures:tuple[int,int],turn:int,num_moves:int)->tuple[int,int]:
    try:
        ...
        score, best_move, count = minimax(board, depth, True, captures, turn, last_move, before_last_move, num_moves)
