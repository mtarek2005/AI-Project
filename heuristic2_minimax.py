import math
from heuristic2 import evaluate_defensive_move
...
def minimax(board, depth, maximizing_player, captures, turn, last_move:tuple[int,int], before_last_move:tuple[int,int], num_moves:int, alpha=-math.inf, beta=math.inf):
    ...
    inv_color=3-turn
    color=turn
    possible_moves = [pmove_s[0] for pmove_s in sorted([(pmove,evaluate_defensive_move(pmove[0], pmove[1], board, inv_color if maximizing_player else color, last_move)) for pmove in possible_moves],key=lambda x:x[1])]
